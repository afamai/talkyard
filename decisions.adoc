...
=== Pub API: Avoid GraqhQL. Use Ty's custom REST + JSON

GraphQL actually looks a bit nice, I start thinking, now when working with
Ty's Search, List and Get API. However GraphQL doesn't seem to bring any
particular benefits to Ty, as of now, 2021-02.

GraphQL could-have-been pros:
- Maybe simpler for people who know GraphQL already to extract
  data from Talkyard. Or maybe not — no one has ever asked about this.
- No need to design Ty's own API query and responses JSON structures.
  Maybe simpler to write GraphQL schema to Scala and Postgres get-the-thing
  mappings — or maybe not. And, this now mostly done anyway, so doing in
  GraphQL too would be dupl work, right.
...
- DoS attack risk? Clients could construct GraphQL queries that ask for
  ...
  or db queries, or custom regular expressions, would also be risky.
...
A supposed REST + JSON cons from https://www.howtographql.com/basics/1-graphql-is-the-better-rest/[here]:

>  With REST, you have to make three requests to different endpoints
> to fetch the required data. You’re also overfetching since the endpoints
> return additional information that’s not needed
...
So, for now, use Talkyard's Get, List and Search API:s, but not GraphQL.
...
some nice more-type-safe ways. Or maybe shouldn't even be in the Scala app,
maybe instead a separate server in Rust reading from a read-only replica rdb.

(However, for backoffice apps, could make sense with some GraphQL lib that
connects directly to Postgres? And is accessible only to trusted backoffice apps.
Websearch for "graphql postgres".)
