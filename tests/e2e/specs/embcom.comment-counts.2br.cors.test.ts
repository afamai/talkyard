...
describe(`embcom.manual.2br.test.ts  TyTE2EEMBCOMMAN`, () => {
  ...
  it(`construct site`, () => {
    ...
    forum.siteData.pageIdsByAltIds[pubPagePath] = pubPageId;
    ...
    forum.siteData.pageIdsByAltIds[staffPagePath] = staffPageId;
  ...
  it(`Owen creates embedding pages`, () => {
    ...
    fs.writeFileSync(`${dir}${pubPagePath}`, makeHtml('pubpg', '#252', pubPageId));
    fs.writeFileSync(`${dir}${staffPagePath}`, makeHtml('staffpg', '#500', staffPageId));
  ...
  it(`... and a blog post index page`, () => {
    ...
    function makeHtml(): St {
      return utils.makeBlogPostIndexPageHtml({ localHostname,
        ...
        urlE: pubPagePath,     // everyone can see num comments
        urlF: staffPagePath,   // only staff can see num comments
  ...
  it(`Maria goes to embedding page one-comment`, () => {
    maria_brB.go2(embeddingOrigin + '/page-one-comment.html');
  });
  it(`... posts a comment`, () => {
  ...
  it(`Maria goes to two-comments`, () => {
    maria_brB.go2(embeddingOrigin + '/page-two-comments.html');
  });
  ...
  it(`... no ok count appears`, () => {
    const numOk = maria_brB.count('.ty_NumCmts-Ok');
    assert.eq(numOk, 0);
  });
  ...
  it(`... just some intentionally incorrect links failed`, () => {
    const numOk = maria_brB.count('.ty_NumCmts-Err-TyEPGNF');
    assert.eq(numOk, numBlogPostLinks - numBlogPostLinksValid);
  });
  ...
  it(`... moves it to Cat A — it'll be pulic`, () => {
    owen_brA.go2('/emb-coms-page-staff-cat');
  ...
  it(`... and one less link failed`, () => {
    const numOk = maria_brB.count('.ty_NumCmts-Err-TyEPGNF');
    assert.eq(numOk, numBlogPostLinks - numBlogPostLinksValid - 1);
  });
