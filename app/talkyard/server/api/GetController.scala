...
class GetController @Inject()(cc: ControllerComponents, edContext: EdContext)
  ...
  def getThingsPubApiImpl(request: JsonPostRequest): Result = {
    ...
    val getAnyPages = (getQueryJson \ "getPages").asOpt[Seq[Ref]]
    if (getAnyPages.nonEmpty) {
      getPagesImpl(request, getAnyPages.get, pretty = pretty)
    }
    else {
      throwBadReq("TyE5603MRD5", "Get what?")
    }
  }
  ...
  private def getPagesImpl(request: JsonPostRequest, pageRefs: Seq[Ref], pretty: Bo)
    ...
    throwUnimplementedIf(pageRefs.size >= 40,
          "TyE603MRT4", "Currently at most 40 at a time")

    def notFoundMsg(embUrl: St, pageId: PageId = NoPageId): St = {
      s"No page with embedding url $embUrl"
    }

    val urlAndIds: Seq[(St, PageId) Or ErrMsg] = pageRefs map { ref: St =>
    ...
    val topicsOrErrs = urlAndIds map { urlAndIdOrErr =>
      urlAndIdOrErr flatMap {
        case (url, pageId) => dao.getPagePathAndMeta(pageId) match {
          ...
          "thingsOrErrs" -> JsArray(
            topicsOrErrs.map({
              case Good(pagePathAndMeta) =>
                Json.obj(
                    "numOrigPostLikeVotes" -> pagePathAndMeta.meta.numOrigPostLikeVotes,
                    "numRepliesVisible" -> pagePathAndMeta.meta.numRepliesVisible)
              case Bad(errMsg) =>
